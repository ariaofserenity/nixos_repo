{config, pkgs, lib, inputs, user, ...}:

 {
  users.users.${user} = {
     isNormalUser = true;
     description = "${user}";
     initialHashedPassword = "$6$5mdi04UuXZNPWVXU$rwSZFrX2tggxwuUTmJtUihty0ps815sGRpjvnURwWjSBMgPRzVct6tMkyN3zZgZsGwbTR.eN3ji6i.hfFSOCh1";
     extraGroups = [ "wheel" "docker" ]; 
     openssh.authorizedKeys.keys = [ 
   "ssh-rsa  aria@zemuria"
   "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKcKrN9QYfMwtym9GQoQGb/zS/HNUUpnjyrx1wPttSMn aria@elysium"
   "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEZQxe+XmdZx4qleCvcOlEMEWQsbbsxz7W6M++GOk9q5 aria@valimar" ];  
     packages = with pkgs; [
       vim
       tree
       traceroute
       htop
      ];
   };

}